...
### Added
- `hardware/esp32-relay/esp32s3_bitchat_relay/esp32s3_bitchat_relay.ino` — NimBLE-based transparent GATT-server relay firmware for ESP32-S3 (N16R8) boards, protocol-compatible with `AppConstants.Mesh.Gatt` / `BinaryProtocol.kt`: parses v1 (13-byte) and v2 (15-byte, route-aware) headers correctly, decrements only the TTL byte and forwards packets otherwise byte-for-byte (packets are signed + padded), notifies subscribed phones instead of scanning/connecting from inside the write callback, and offloads relay fan-out to `loop()` via a FreeRTOS queue so the BLE host task is never blocked
- Three-mode RST-cycled operation persisted in NVS flash (`Preferences`), surviving both RST-button presses and power cycles: mode 0 legacy-1M-only, mode 1 adds a Coded-PHY extended advertising set with S=2 connection PHY preference, mode 2 same with S=8 preference — mirrors the app's additive/never-substitutive Coded-PHY design (`BluetoothGattServerManager.updateMeshDensity`) by always keeping the 1M advertisement on air as a discovery floor. NVS was chosen over RTC memory after observing this board's RST button is hardware-classified as a POWERON reset all the way down at the ROM bootloader (`rst:0x1 (POWERON)`), which ESP-IDF does not guarantee RTC-memory retention across
- Onboard WS2812 LED shows current mode (green/blue/red) with a white flash per relayed packet, for at-a-glance range-mode and activity feedback
- FIFO-evicting dedup cache (senderID + timestamp + type key) replacing a clear-on-overflow set, to avoid re-relaying in-flight packets after the cache fills
- `BitchatMessage.hopCount` — number of relay hops (phone or ESP32 relay hardware) a message crossed before reaching this device, computed in `MessageHandler` as `AppConstants.MESSAGE_TTL_HOPS - packet.ttl` at the point of receipt for both broadcast and private messages (text and file); rendered as a "· N hops" badge after the timestamp/PoW badge in `ChatUIUtils.formatMessageAsAnnotatedString` and `formatMessageHeaderAnnotatedString`, hidden for the common 0-hop (direct-connection) case
- `esp32s3_bitchat_relay.ino`: periodic per-connection RSSI logging (`logRssi()`, every 3s while any peer is connected, via `ble_gap_conn_rssi`) for range testing across the three PHY modes — tracks connected peers in a `PeerConn` vector updated from `onConnect`/`onDisconnect`
- `esp32s3_bitchat_relay.ino`: `rssiToDistanceMeters()` — log-distance path-loss estimate printed alongside each RSSI reading as `~N.Nm`; documented as an approximate near/medium/far indicator, not a precise measurement, given multipath/body-blocking fade. `RSSI_REF_AT_1M` and `PATH_LOSS_EXPONENT` are per-mode arrays (indexed by the existing `mode`) rather than single globals, since the phone's controller may trim TX power on Coded PHY and this radio's RSSI reporting can carry a small per-PHY offset — each mode is calibrated independently
- `esp32s3_bitchat_relay.ino`: `timestampStr()` prefixes `RSSI`, `Connected`, `Disconnected`, `PHY pref`, and `Stats` log lines with a wall-clock `HH:MM:SS`, preferring three sources in order: (1) `harvestPhoneTime()` — the epoch-millisecond timestamp every packet already carries in its header (`BinaryProtocol.kt`'s `timestamp` field, bytes 3-10), harvested for free from ordinary relay traffic in `preprocessPacket()` with no new characteristic or app change, self-correcting on every well-formed packet regardless of dup/TTL outcome; (2) one-time NTP sync at boot (`syncTimeThenDisableWifi()`) if `WIFI_SSID` is configured, with WiFi fully powered off again (`WiFi.mode(WIFI_OFF)`) before BLE starts since the S3 shares one 2.4GHz radio between WiFi and BLE; (3) `+123.4s` elapsed-since-boot fallback requiring no network or connected phone at all
- `hardware/esp32-relay/README.md` — setup/onboarding doc for the relay firmware: NimBLE-Arduino 2.x + manual `nimconfig.h` extended-advertising setup (including how to locate a non-default sketchbook path via build-cache `.d` dependency files), mode/LED table, serial log format reference, RSSI/distance calibration procedure, and known limitations (peripheral-only, no ESP-to-ESP bridging, no multi-anchor positioning)
